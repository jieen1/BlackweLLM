"""CUDA Graph wrapper for the DFlash draft model's speculative decode step.

任务#41 (vLLM removal plan, 阶段8): this file used to also hold
``DFlashVerifyCudaGraph``, a FlashInfer-based main-model verify graph
(the reason this file was on both the vLLM and FlashInfer direct-import
ledgers in tests/test_vllm_dependency_boundary.py). Re-verified via git
history (not just carried forward from the 阶段0 audit's suspicion) that
it has had zero real callers since commit d4354e939e9 (2026-07-25):
the active verify-CUDA-graph path is ``LagunaCudaGraphVerify``
(runtime/backends/laguna_cuda_graph.py, sparkinfer-only, no FlashInfer/
vLLM import at all), wired from ``DFlashEngine._capture_verify_cg``
(runtime/backends/laguna_dflash.py) since that commit. Deleted --
this file is now sparkinfer-only, draft-model-only.

Per speculative step:
1. Main decode (M=1): LagunaCudaGraphDecode (existing)
2. combine + precompute_context_kv: eager (small ops)
3. Draft forward (M=16): CUDA Graph with prefill wrapper (this file)
4. Verify forward (M=16): CUDA Graph (runtime/backends/laguna_cuda_graph.py)
"""
...
class DFlashDraftCudaGraph:
    ...
    def _build_metadata_and_forward(self) -> torch.Tensor:
        ...
        # BFAttention instances too (LagunaDFlashEngine._patch_draft_
        # sparkinfer), which read this context instead of vLLM's real
    ...
    def replay(self, slot: int, bonus_token: int, kv_len: int) -> list[int]:
        ...
        if os.environ.get("QSR_DEBUG_LOGITS_LIGHT") == "1":
