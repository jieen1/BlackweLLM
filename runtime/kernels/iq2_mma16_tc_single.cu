...
extern "C" QSR_EXPORT void iq2_mma16_tc_launch_single_dynamic(
    const int8_t* xq, const float* xs, const uint8_t* packed,
    const int64_t* eids, const int64_t* grid, const int32_t* ksigns,
    const int32_t* expert_bounds, float* out,
    int E, int ROWS, int COLS, int STRIDE, cudaStream_t stream)
{
    const int smem_bytes = 32 * 256 + 32 * 8 * 4;
    cudaFuncSetAttribute((const void*)iq2_mma16_tc_single_dynamic_kernel,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes);
    dim3 block(128);
    dim3 gridc(E, ROWS / 32);
