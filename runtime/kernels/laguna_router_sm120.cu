...
__global__ __launch_bounds__(32 * kWarpsPerBlock) void laguna_router_bf16_kernel(
    ...
    const __nv_bfloat16* row_logits = logits + static_cast<size_t>(row) * kExperts;
    float scores[kValuesPerLane] = {
        __bfloat162float(row_logits[lane * 4]),
        __bfloat162float(row_logits[lane * 4 + 1]),
        __bfloat162float(row_logits[lane * 4 + 2]),
        __bfloat162float(row_logits[lane * 4 + 3]),
        __bfloat162float(row_logits[lane * 4 + 128]),
        __bfloat162float(row_logits[lane * 4 + 129]),
        __bfloat162float(row_logits[lane * 4 + 130]),
        __bfloat162float(row_logits[lane * 4 + 131]),
    };
    float scores_for_choice[kValuesPerLane];
...
extern "C" __attribute__((visibility("default"))) int qsr_laguna_router_bf16(
    const __nv_bfloat16* logits,
    const float* correction_bias,
    float* topk_weights,
    int32_t* topk_ids,
    ...
    const dim3 block(32, kWarpsPerBlock);
    const dim3 grid((num_rows + kWarpsPerBlock - 1) / kWarpsPerBlock);
    laguna_router_bf16_kernel<<<grid, block, 0, stream>>>(
        logits, correction_bias, topk_weights, topk_ids, num_rows);
    const cudaError_t status = cudaPeekAtLastError();
    return status == cudaSuccess ? 0 : static_cast<int>(status);
}
