...
> | 项 | 量级 | 性质 |
> |---|---:|---|
> | **额外一次完整前向推进 anchor** | **~194–240ms** | ⚠️ **e2e 脚本结构的产物，可折进 verify 块** |
> | MTP 起草 | ~73ms | 算法固有 |
> | 接受率 15% / 35% | — | 决定每轮能提交几个 token |
>
> - [ ] **B3-a：把 anchor 推进折进 verify 块** —— 单项约占每轮成本的 35–45%，
>       且被明确判定为**脚本结构的产物而非算法必需**。这是目前唯一的大杠杆。
> - [ ] **B3-b：接受率**（prose 15%、code 35%）—— 即使耗时全部解决，15% 的接受率也
>       撑不起投机。要先弄清是草稿头质量问题还是 K/采样配置问题。
>
> **在 B3-a 落地并重测之前，不应把 MTP 接进服务路径**——接进去会让 Qwen3.6 变慢。
