...
>       本项目反复验证过的规律）。
> - [ ] **B3-b：接受率**（prose 15%、code 35%）—— **现在是唯一还有杠杆的地方**：即使耗时
>       全部解决，15% 的接受率也撑不起投机。要先弄清是草稿头质量问题还是 K/采样配置问题。
>
> **B3-a 没有翻正 MTP，且经排查后已确认没有低风险的翻正手段** —— **不应把 MTP 接进服务
> 路径**：接进去会让 Qwen3.6 变慢（0.72×/0.91×）。
> ⚠️ 也**不要**再把 GDN 侧的优化当成翻正手段：硬上限已经排除了它。
