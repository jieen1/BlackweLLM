...
- [ ] 贪心输出 **bit-exact**
- [ ] 性能不低于基线 3%——**仅对 fox-4K 353–357 / galaxy-4K 395–401 / code-4K 341–359 tok/s
      成立**（2026-08-02 多次重启逐一复现，可信）
- [ ] ⚠️ **fox-64K 暂不作为验收判据。** 2026-08-02 实测：同一负载**调用在序列中的位置**
      能让吞吐摆动约 60%（首次调用 13.7s ≈ 480 tok/s，其后每次 ~22.0s ≈ 298 tok/s，
      steps 数完全相同、稳定不漂移）。而 07-31/08-01 的记录只有汇总数字、没有逐轮调用
      顺序，**无从知道 353–368 是哪种状态下测出的**。已排除：verify 容量修复（targeted
      回退后仍 298）、前缀缓存（该 benchmark 全程 `enable_prefix_cache=False`，代码路径
      执行不到）。待查线索：PyTorch CUDA caching allocator 对 ~64K KV 区域尺寸类的首次
      与后续分配行为差异（通用问题，与 4K 不显现相符）。
      **重建这条基线时必须发布逐轮序列，不能只发汇总。** 见 Track F。
- [ ] 接受率不回归（96.3–100%）
...
> ---
>
> 🎯 **由此确定的下一步（这是本条唯一还有杠杆的地方）**
>
> 三次"这就是瓶颈"的判断都被后续测量推翻了：先是 sparkinfer 的 kernel 启动开销
> （真值 ~0.8–1.0ms 而非 ~6.8ms），再是大投影（批处理只把 e2e 推到 0.91×/0.79×）。
> **硬上限本身给出了答案**：把 GDN verify 成本归零也只到 prose 1.09× / code 0.905×，
> 所以**决定项从来不在 GDN 侧**。
>
> 每轮 505–685ms 的构成里，最大的单项是：
>
> | 项 | 量级 | 性质 |
> |---|---:|---|
> | **额外一次完整前向推进 anchor** | **~130–240ms**（同进程内 K=1 探针 128ms，跨会话【推算】194–240ms） | ~~⚠️ e2e 脚本结构的产物，可折进 verify 块~~ 🔴 **已实测排除，见下** |
> | MTP 起草 | ~73ms | 算法固有 |
> | 接受率 15% / 35% | — | 决定每轮能提交几个 token |
>
> - [x] ~~**B3-a：把 anchor 推进折进 verify 块**~~ —— 🔴 **已排查，结论是"不能"，不是脚本结构
>       的产物，是算法必需**（2026-08-02，`work/b3a-anchor-20260802`，未合并 main，见
>       [`../notes/2026-08-02-b3a-anchor-fold-verdict.md`](../notes/2026-08-02-b3a-anchor-fold-verdict.md)）：
>       每轮新确定的 anchor（拒绝分支的纠正 token / 全接受分支的 bonus token）按
>       `runtime/mtp_accept.py::determine_accept_reject_from_predictions` 的定义**永远不等于**
>       任何已经跑过 `verify_forward` 的草稿 token——它的**值**从 verify 的 logits 免费拿到，
>       但它的**隐状态**（MTP 起草下一轮所需的 `prev_hidden` 种子）只能靠把它自己塞进 64 层
>       backbone 再跑一次才能拿到，这是"隐状态是该 token 自己的前向输出"这条数学关系决定的，
>       不是脚本怎么组织循环的问题。唯一的"零风险"规避（把 confirm 从普通 `forward()` 换成
>       `verify_forward(K=1)`，即字面意义"走 verify 代码路径"）已实测：**不但没变快，反而慢
>       9%**（128.28ms→139.78ms，同进程连续测量，`scripts/b3a_probe_confirm_cost.py`）。
>       唯一数学上真正免费的分支（K 个草稿全部接受 **且** MTP 自己多起草的第 K+1 个猜测恰好
>       等于目标模型的 bonus 预测）在当前接受率下发生概率 ~1e-4 到 1e-7 量级，不构成可交付优化。
>       **端到端重测（零代码改动，纯粹重新测量）：prose 0.72×、code 0.91×——仍然 <1.0×**，
>       与旧【推算】的 0.784×/0.704× 不是同一次测量，不可直接比较（GPU 热状态漂移，
>       本项目反复验证过的规律）。
> - [x] **B3-b：接受率**（prose 15%、code 35%，K=8 时）—— ✅ **已测，K 曲线翻转了结论**
>       （2026-08-02，`work/b3b-accept-20260802`，未合并 main，见
>       [`../notes/2026-08-02-b3b-acceptance-rate-vs-k.md`](../notes/2026-08-02-b3b-acceptance-rate-vs-k.md)）：
>       接受率随 K **单调下降**（prose 0.62→0.21，code 0.80→0.31，K=1→8，N=128 实测），
>       但加速比**不是**单调的——K=1 起草+verify+推进 anchor 的固定开销摊不到 1 个 token 上
>       （0.80–0.82×），K=8/16 接受率跌得比 K 涨得快（0.88×/0.65×），**中间 K≈3–6 是两头都
>       还没输的甜蜜点：prose 1.06–1.11×、code 1.27–1.41×，两个 prompt 都实测 >1.0×**——
>       同一份权重、零代码改动，只把写死的 K=8 换成 K=4/5。这是整条 B3 investigation
>       第一次量出真实 >1.0× 的 speedup。teacher-forced 单步测量（159 个位置/prompt）显示
>       草稿头本身准确率 63–82%、不算差，K=8 时 15%/35% 的低接受率**主要是链式复合误差
>       （几何衰减，K 越大轮次越吃亏），不是头本身弱到没法用**——决定性证据是"调 K 就能翻正
>       speedup"这件事本身：如果是纯头质量问题，调 K 不可能翻正任何东西。
>       ⚠️ **这仍然是独立 eager 脚本（单 slot，无 CUDA Graph，无连续批处理，不经过
>       `ServerEngine`）的数字**，和本节此前所有数字同一个局限——"K=4/5 更好"这个结论可信，
>       "1.38×/1.41× 这个具体数字能直接当生产 SLO"不可信，下一步是拿这个结论去
>       `ServerEngine` 里复测，不是直接把 MTP 接进服务路径。
>       同一份笔记也用 gap error（不是 token 相等）判定了 code prompt 的 `tokens_match=False`
>       复现：`gap_error=0.125`（bar 0.5）、`tie_slack_ulps=1.0`（bar 32.0，字面意义上的
>       bf16 平局），三项数值判据全部落在 B1-R"正确"区间，坐实上一轮"良性 kernel 调度分歧"
>       的预判。
>
> **B3-a 没有翻正 MTP（在 K=8 下），B3-b 在 K≈3–6 下测出了正翻转，但两者都还没在
> `ServerEngine` 里验证过** —— ~~不应把 MTP 接进服务路径~~ **改为：值得再花一次 GPU 窗口
> 把 K=4/5 接进 `ServerEngine` 复测；在那之前，K=8 默认值本身已确认是错的选择（无论最终
> 结论如何，都不该继续用它）**。
> ⚠️ 也**不要**再把 GDN 侧的优化当成翻正手段：硬上限已经排除了它，K 曲线的翻转和 GDN 成本
> 无关（同一套 GDN 实现，只是 K 变了）。
> ⚠️ **也不要把"近似 anchor 种子"（用错一格位置的隐状态顶替，绕开这次前向）当成翻正手段**：
> 那是未经验证的 MTP 起草质量改动，`work/b3a-anchor-20260802` 的排查明确判定为越界，
> 需要专门的训练分布匹配验证才能考虑，不是"脚本结构"层面能做的事。
>
> 📌 与 Laguna 的对比说明了差距的一部分原因，但不是全部:Laguna 的 DFlash 投机接受率
> 96.3–100%，用的是专门训练的独立 6 层 draft 模型（不是 checkpoint 自带的单层 MTP 头），
> 且 Laguna **没有 GDN 层**，verify 不付这份代价，两者不可直接比接受率数字本身。但
> B3-b 测出 Qwen3.6 MTP 头的 K=1 单步接受率是 52–80%（teacher-forced 准确率 63–82%）——
> **不是坏到没法用**，K=8 时 15%/35% 的低接受率大头是"K 选错了、复合了 8 次"，不是头质量
> 差到量级不对。**"Laguna 的投机有效，Qwen3.6 也该有效"这个推断依然不成立**（架构真实不同），
> 但"Qwen3.6 的 MTP 头质量差到投机注定不划算"这个更强的推断，被 B3-b 的 K 曲线证伪了。
...
| P0-E 第 7–8 步 | 第 6 步完成 + GPU | 🟡 第 7 步（A3 协调者，爆炸半径最大）**设计已完成并拍板**，见 [`a3-cache-coordinator-design.md`](a3-cache-coordinator-design.md)（决策点 1/2/3/5 已定，4 属 Track B）；实施是下一个 |
