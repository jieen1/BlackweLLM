...
## 7.9 协调者裁定（2026-08-02）

§8 提出的三个待拍板事项，全部已定。逐条理由如下。

### ✅ 决策 1：采纳 B1-R，并判定 **B1 通过**

采纳的决定性理由**不是**"当前实现能过"，而是三条：

1. **方法合法性是实测出来的，不是论证出来的**：用我们自己的 token 做步锁强制解码，
   与自由生成**逐位相同**（§5.1）。这一条恰好堵住了我先前给出的那个否决理由——
   `notes/2026-08-02-b1-greedy-alignment-fails.md` 里说"教师强制类方法对 Qwen3.6 无效"，
   指的是**重新 prefill** `prompt + oracle[:i]`（走 chunk 算法冒充 recurrent，相差约 30 ULP）。
   步锁**从不重新 prefill**，每一步两侧都是 `seq_len==1`。两者不是一回事，B1-R 站得住。
2. **判据有区分力，是注入实验证明的**：7 个噪声底之上的注入**全部变红**，每个红 5–10 条 bar；
   红绿两组不重叠（6.8× 间隔）。**抓不到注入 bug 的判据是废的**——这条已被排除。
3. **阈值来自噪声底而非"当前能过什么"**：τ = 0.25 × 2 ULP。若按"当前能过"反推会得到 >10，
   那才是放宽。

同样重要的是**保持绿色的 6 个配置各有可证明的理由**（§5.5）：均匀 RoPE 位置偏移是**恒等变换**
（RoPE 只依赖 i−j）；0.1% 的 GDN 状态衰减在 bf16 下**格式表示不出来**（1.0 附近 ULP = 0.39%）。
这不是漏检，是被测对象本身不存在。

**判定 B1 通过，但 `IMPLEMENTED_BACKENDS` 不翻。** B1 的范围是"eager、batch=1 的正确性"，
可服务性是 B2 的事，两者不能混。§8.2 的两个缺口（NVFP4 反量化未覆盖、全部测量 ≤300 位置）
原样记录，不因为判定通过就淡化。

### ✅ 决策 2：两个 R3 缺陷分开处理

- **`qwen36_capture.py` 的 layer-63 前后归一化错配** —— **就地修**。它是局部的、只影响 Qwen3.6，
  且证据已经拿到（RMS 迹 3.07→5.03 across 56–62，然后 63 处 **1.97**）。下一个 GPU 窗口做。
- **`thresholds.py` 的 `HIDDEN_STATE.max_rel_abs_error=0.02`** —— **进 C8 债务清单，不现在动**。
  理由是这个常数**与 Laguna 共用**，而 Laguna 是唯一生产模型。实测它对真实 bf16 残差流不可达
  （0.33 @ cos 0.9999957）说明它零区分力，但"零区分力"和"改了会误伤"是两回事，
  需要一次覆盖两个模型的重新标定。

### ✅ 决策 3：保留"分歧率"这条弱 bar，但**永不单独触发**

实测它确实弱（控制组 5 次翻转 vs 最弱 bug 11 次，约 2.7σ，两个分布几乎相接）。
但**保留的成本为零，丢弃的代价是丢信息**——它作为佐证信号仍然有效，只是不能单独定罪。
文档已按此实现，维持。
