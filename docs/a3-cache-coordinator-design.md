...
> **状态（2026-08-02，协调者裁定）**：决策点 1/2/3/5 **已拍板**，逐条理由见下方各条开头的
> 「✅ 已定」段落。决策点 4 属 Track B/B2 范围，本轮不决定。
>
> 这四条都是工程判断而非战略判断——爆炸半径落在"一个协议签名 + 一个调用点"这个量级，
> 且四条的裁定都与本文档的推荐一致（推荐的论证已被独立复核，不是照单接受）。
...
**✅ 已定：(b)，且已落地。** `architecture.md` §2.4 与 `roadmap.md` S4 的"残迹"措辞已改成
"休眠原语"。触发这次更正的是一个独立发现：`architecture.md` §2.4 原本把当前前缀缓存描述成
"内容寻址：块级哈希 + 引用计数 + LRU"，而**生产路径对 `BlockPool`/`hash_block_tokens` 的
调用方为零**，同时它有 44 个通过的测试。所以"残迹"（暗示待清理）确实是错词——准确状态是
**测试充分但无人调用**，这跟"休眠原语"是同一件事。
...
**✅ 已定：(a) 原地改。** 今天只有一个实现者、一个调用点，`protocol.py:42-46` 自己的文档
就预告了"这三个调用点下一次被摸到时"是改形状的时机——现在正是。决定性的理由是 (b) 的代价
被低估了：两个语义相近的方法外加"哪个是权威值"的问题，正是本仓库反复吃亏的那类歧义
（`slot_kv_len` 被当成映射实际是列表 → `/metrics` 忙时 500；N8 的 `try/except` 把错误配置
伪装成正常运行、三年无人发现）。而 (a) 的迁移成本只会随实现者增多而变大，不会变小。
若 Track B 落地时发现第二个 backend 真的需要纯 `int` 语义作向后兼容，退回 (b) 的代价是
7-a 返工一步——可接受，且届时的信息比今天多。
...
主题的具体案例），`(b)` 引入的"两份数据必须同步"本身就是一个新的、不必要的不变量。

**✅ 已定：(a)。** 只读 `ArchitectureSpec.needs_two_cache_families`，不在
`BackendCapabilities` 加派生字段。"两份数据必须保持同步"是一个凭空新增的不变量，而本仓库
对"新增不变量"的历史成绩不好——每一条最后都是靠事故而不是靠断言被发现的。
`/metrics` 若需要统一入口，在 7-a/7-d 落地时加薄适配层，不改架构选择。但
...
**✅ 已定：(a)，不切换。** 第 7 步是 Track A 里唯一被标注"爆炸半径最大"、也是第一步做不到
"零行为变更"的；前六步全靠这一点降风险。给它再叠一个真正的行为变更（跨请求 KV 共享，今天
不存在）是拿唯一的生产模型冒险换一个 Track B 并不需要的收益。
