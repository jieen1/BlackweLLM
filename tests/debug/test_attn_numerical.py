...
vc = EngineArgs(model=MODEL, dtype="bfloat16", max_model_len=4096,
...
for name in sorted(captured.keys()):
    ...
    print(f"  q norm={q.float().norm():.2f} k norm={k.float().norm():.2f} v norm={v.float().norm():.2f}")
    ...
    ref = F.scaled_dot_product_attention(
        ...
        is_causal=True
    ).squeeze(0).transpose(0,1)  # [M, nh, hd]
    ref_flat = ref.reshape(M, nh*hd).to(torch.bfloat16)

    si_flat = si_out  # [M, nh*hd]
    cos = F.cosine_similarity(si_flat.float().reshape(1,-1), ref_flat.float().reshape(1,-1)).item()
    ...
    print(f"    si norm: {si_flat.float().norm():.2f}  ref norm: {ref_flat.float().norm():.2f}  ratio: {ref_flat.float().norm()/si_flat.float().norm():.4f}")

    # Per-position
    for i in range(min(M, 5)):
        pc = F.cosine_similarity(si_flat[i].float().reshape(1,-1), ref_flat[i].float().reshape(1,-1)).item()
        print(f"    pos{i}: cos={pc:.4f} si={si_flat[i].float().norm():.3f} ref={ref_flat[i].float().norm():.3f}")
    ...
    ref_fp8 = F.scaled_dot_product_attention(
        q.float().transpose(0,1).unsqueeze(0),
    ...
    cos_fp8 = F.cosine_similarity(si_flat.float().reshape(1,-1), ref_fp8_flat.float().reshape(1,-1)).item()
    cos_quant = F.cosine_similarity(ref_fp8_flat.float().reshape(1,-1), ref_flat.float().reshape(1,-1)).item()
    print(f"\n  [Isolation]")
