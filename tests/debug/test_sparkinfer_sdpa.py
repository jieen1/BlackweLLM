...
k_expanded = (
    k_bf16.unsqueeze(2)
    .expand(-1, -1, NUM_Q_HEADADS // NUM_KV_HEADS, -1)
    .reshape(SEQ_LEN, NUM_Q_HEADADS, HEAD_DIM)
)
v_expanded = (
    v_bf16.unsqueeze(2)
    .expand(-1, -1, NUM_Q_HEADADS // NUM_KV_HEADS, -1)
...
kv_cache = torch.zeros(
    NUM_PAGES, 2, PAGE_SIZE, NUM_KV_HEADS, HEAD_DIM, dtype=torch.uint8, device=device
)
...
ws = PagedAttentionWorkspace.for_tensors(
    mode="extend", q=q_bf16, k_cache=si_k_cache, v_cache=si_v_cache, use_cuda_graph=False
)
...
cos_sim = F.cosine_similarity(
    si_output.float().reshape(1, -1), ref_output.float().reshape(1, -1)
).item()
...
if cos_sim > 0.99:
    ...
    for i in range(SEQ_LEN):
        pos_cos = F.cosine_similarity(
            si_output[i].float().reshape(1, -1), ref_output[i].float().reshape(1, -1)
        ).item()
        print(
            f"    pos {i}: cos={pos_cos:.4f} si_norm={si_output[i].float().norm():.2f} ref_norm={ref_output[i].float().norm():.2f}"
        )
