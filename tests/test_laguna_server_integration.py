...
def test_qwen_non_thinking_chat_response_keeps_html_in_content(monkeypatch):
    """Qwen3.6/3.8 close the prompt's think block when thinking is disabled.

    The generated tokens therefore begin with visible content, not with a
    reasoning body.  A process-level thinking-capable default must not make
    the response parser synthesize a second, unclosed ``<think>`` block and
    move the entire answer into ``reasoning_content``.
    """
    from server import app as server_app

    raw = "<!doctype html><html><body>visible</body></html>"
    _patch_server_app(monkeypatch, server_app, _fake_engine_returning(raw), raw)
    monkeypatch.setattr(server_app, "SERVER_THINKING_CAPABLE", True)
...
def test_qwen_non_thinking_stream_keeps_html_in_content_delta(monkeypatch):
    """The request-scoped Qwen mode must also reach the SSE parser."""
    from server import app as server_app

    raw = "<html>visible</html>"
    ...
    async def _run():
        response = await server_app.chat_completions(
            server_app.ChatCompletionRequest(
                messages=[{"role": "user", "content": "build a page"}],
                chat_template_kwargs={"enable_thinking": False},
                stream=True,
    ...
    reasoning = "".join(
        payload["choices"][0]["delta"].get("reasoning_content", "") for payload in payloads
    )
    assert content == raw
    assert reasoning == ""
...
def test_qwen_non_thinking_compat_endpoints_keep_html_in_content(monkeypatch, endpoint):
    ...
    raw = "<html><body>visible</body></html>"
    seen_kwargs = []

    async def _tokenize_chat(*_args, **kwargs):
        seen_kwargs.append(kwargs.get("chat_template_kwargs"))
        return [0]
    ...
    if endpoint == "anthropic":
        response = asyncio.run(server_app.anthropic_messages(_FakeRequest()))
        ...
        assert "debug_reasoning_content" not in response
