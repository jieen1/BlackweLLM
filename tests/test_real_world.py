...
Usage: python tests/test_real_world.py [base_url]

Everything below used to execute at module scope (HTTP calls included) and
end with a bare ``sys.exit`` -- which is exactly why ``tests/conftest.py``
excludes this file from collection: importing it for any reason (even by
another tool wanting to reuse a check) fired real network I/O and could
kill the importing process. All of it now lives in ``run()``, called only
from the ``__main__`` guard below or explicitly by a caller (e.g.
scripts/c_live_smoke.py) that wants these checks as a subroutine.
"""
...
def run(base_url: str | None = None) -> tuple[int, int]:
    """Run every check against ``base_url`` (default: argv[1], else
    http://127.0.0.1:8000 -- matches the original script's argv handling)
    and return (passed, failed) instead of printing a verdict and calling
    ``sys.exit``."""
    base = base_url or (sys.argv[1] if len(sys.argv) > 1 else "http://127.0.0.1:8000")
    host = base.replace("http://", "").split(":")[0]
    port = (
        int(base.replace("http://", "").split(":")[1]) if ":" in base.replace("http://", "") else 80
    )
    ...
    for turn in range(5):
        ...
        for b in r["content"]:
            if b.get("type") == "tool_use":
                results.append(
                    {"type": "tool_result", "tool_use_id": b["id"], "content": "12:00 PM"}
                )
    ...
    check(
        f"health during gen: avg={avg_lat:.0f}ms < 1000ms", avg_lat < 1000, f"avg={avg_lat:.0f}ms"
    )
    check(
        f"health during gen: max={max_lat:.0f}ms < 2000ms", max_lat < 2000, f"max={max_lat:.0f}ms"
    )
    ...
    # ============================================================
    results3: list[tuple[int, dict] | None] = [None, None, None]
    ...
    all_ok = all(r is not None and r[0] == 200 for r in results3)
