...
class TestSlotViewsAliasThePool:
    ...
    def test_kv_view_is_the_slots_own_contiguous_page_range(self) -> None:
        pool = _pool()
        cache = pool.slot_state(2).attn_caches[0]
        assert cache.num_pages == pool.pages_per_slot
        cache.k_cache.fill_(3.0)
    ...
    def test_local_and_global_addressing_reach_the_same_bytes(self) -> None:
        ...
        pool = _pool()
        slot, local_page = 2, 1
        pool.slot_state(slot).attn_caches[0].k_cache[local_page].fill_(5.0)
