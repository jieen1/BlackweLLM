# B1 逐 token 对齐门禁：不通过 —— 而这条门禁本身已作废

日期：2026-08-02 · 状态：🟢 **已结案**（原门禁作废，B1 在替代判据 B1-R 下判定通过）
· 执行：协调者直接跑

> ⚠️ **读这篇之前先知道结局**：本文记录的"不通过"是**对一个已作废门禁**的测量。
> 该门禁要求两个 bf16 实现在 512 步内零 argmax 翻转，而本文正文实测证明分歧发生在
> **1–2 个 bf16 ULP** 上——它要求的东西在这个数值格式下不存在。
>
> 替代判据 **B1-R** 已于同日采纳并判定 **B1 通过**，见
> [`../docs/b1-correctness-criterion.md`](../docs/b1-correctness-criterion.md)。
> 那不是放宽：B1-R 的区分力由注入实验证明（7 个噪声底之上的注入全部变红，红绿两组
> 不重叠），阈值取自噪声底而非"当前能过什么"。
>
> **本文保留，因为它是 B1-R 成立的证据链**——尤其是"分歧处只差 1–2 ULP"这个实测，
> 以及下文那条被自我推翻的误报。
...
- **原门禁已作废**，替代判据 **B1-R** 已采纳，**B1 判定通过**。当时写的"合理的判据可能是
  '首次分歧处必须是近似平局'"方向对了，但**最终采纳的比它更强**：B1-R 的主指标是
  *gap error*，平局规则是它的**推论而非第二个旋钮**（`0 ≤ tie_slack ≤ gap_error` 恒成立），
  因而在"一次翻转都没有"的运行上仍然有信息量——纯平局判据在那种情况下是瞎的。
- **`IMPLEMENTED_BACKENDS` 已翻**，但不是因为 B1：是 **B2 服务化完成**后翻的
  （C-LIVE 67/67，bit-exact 13/13）。B1 判过时明确保持不翻——**B1 管正确性，B2 管可服务性**。
