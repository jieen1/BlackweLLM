...
- [2026-08-06 128K/c4 历史性能追平剖析](2026-08-06-128k-c4-parity-profiling.md) —— 🟢 **按每提交 token 已打平至 ~95%**（4.7 vs 4.5 ms/token）：历史 222.44 tok/s = 45 ms/轮 × 10.04 token（接受率 50.3%），当前 capacity=4 稳态 b4 ≈ 74 ms/轮 × 14.96 token（同 fixture 接受率 91.3%）。差距定位到 **inter-verify GPU 空闲（中位 14 ms/轮，70% 窗口空转）**；GC/engine bookkeeping/split_kv 逐一排除；新增 token-id completions 协议与历史 fixture 直发 harness；已修 verify 最坏容量 chunk 冻结（sparkinfer adaptive chunking 扩展，verify GPU −4.6 ms/轮）；下一步 = draft 侧同款修复 + commit 尖峰 + 轮间空闲
