...
| `2026-08-26-qwen38-flash-next-day0-survey.md` | **Qwen3.8-Flash-Next（125B-A6B，Qwen4 架构预览）day-0 接入调研**——规格核实（125B 主模型 + 51B engram n-gram 表）、vLLM/SGLang/transformers 支持现状（均未合并，但混合注意力/ngram embedding/MTP 三块积木已存在）、96 GB 卡显存算术（FP8 出局、NVFP4+INT4-engram 紧但可行）、对本仓库的四项缺口（hybrid backbone 上的 MoE、engram、多模态、MoE+MTP 回滚）与发布夜行动清单。§6 记录发布前实测与已实现组件：b12x fused MoE 对 E=512 几何 PASS（每层驻留 13.5 GiB、装载峰值 +14.5 GiB/层）、家族路由是 softmax 而非 Laguna 的 sigmoid、路由/MoE 层/专家装载/离线 NVFP4 量化/engram 五个模块已实现并全绿 |
| `2026-08-27-flashnext-runtime-support-plan.md` | **Flash-Next qwen4_exp 自研 runtime 实施方案**——架构/权重差异、P1–P4 路线、MTP/PLE/QSA/CUDA Graph 设计与验收门禁 |
| `2026-08-27-flashnext-sglang-baseline.md` | **Flash-Next 与 SGLang 同提示基线**——性能/接受率/显存数字，以及 CUDA Graph×NVMe PLE、MTP×FlashInfer 的限制与后续验证方法 |
...
| `2026-08-25-qwen38-fullchain-attribution-cold-prefill.md` | **Qwen3.8 DFlash2 128K 全链路 GPU 归因收口**——cold prefill 57% 是 FA2 FP8-KV attention 且已达 cuBLAS 同形状上限的 96%（"20s 消失时间"证伪为分段计时假象）；trtllm-gen/fp8-MMA attention 在 SM120 被架构门关闭（代码级证据）；**B8 实测成立：长 gen aggregate +23%、decode 窗口 +41%、SHA 不变**（capacity=8 + num_slots=8，85.5/97.9 GiB）；剩余杠杆只剩个位数%的融合类苦活 |
