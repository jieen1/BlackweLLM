...
>
> **⚠️ 2026-08-07 晚更正（自检）**：门禁脚本曾有一个我自己的 bug——
> decode 循环的 `pos` 从不递增，导致"64 步后余弦恒定 0.99864638"的
> 假象（每步都在同一位置重算同一 token）。已修（`pos += 1`）。修正后
> 20 步 smoke 的真实数字：logits cos 最差 **0.98**、逐层最差 **0.82**、
> 流一致 **58/63 (92%)**，余弦逐步变化、随 token 振荡。此前的
> "46/51" 门禁 smoke 数据作废；64 步轨迹调试（pos 正确递增）的
> 0.996-0.998/63-64 属实但**只报了末段**，中间 dips（0.98/0.82）被
> 低估。修正后的完整门禁（3×512）在跑，以它为准。
