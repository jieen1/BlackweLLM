...
> ## ⚠️ 关于"性能达标"的更正（同日，见 §九）
>
> 本文 §七 报的 **1.6~2.0x 只在 `capacity=1` 成立**，而且判据是**最弱的那条**：
> 「开投机比不开投机划算」——拿我们自己跟我们自己比，不是跟历史实现比。
>
> 补测并发后结论反转：**`capacity=4` 下 MTP 是 0.49x**，比不开还慢一倍。
> 绝对吞吐距历史实现也差得远。**「达标」这个说法是错的**，详见 §九。
...
## 九、更正：「性能达标」不成立 —— 判据太弱，且并发下是净亏

§七 说 MTP「从 0.79x 变成 1.6~2.0x，转为净胜」。这句话在它自己的口径里没错，
但那个口径**不足以支撑"达标"**，补测之后结论反转。

### (1) 我用的判据是什么

「一轮耗时 < 每轮提交 token 数 × 非投机单 token 耗时」——即**不亏**。
这只回答「在我们自己这套 runtime 上，开投机比不开划算吗」，**不回答这套
runtime 快不快**。对比对象是我们自己，不是历史实现，不是任何外部参考。
而且**只在 `capacity=1` 测过**。

### (2) 并发下是净亏（新测，`mtp_concurrency.py`）

`capacity=4`，4 个并发请求，各 192 token，CG 开，FP8 KV 开，前缀缓存开：

| | 墙钟 | 聚合吞吐 | 客户端错误 | 服务端 traceback |
|---|---|---|---|---|
| MTP off | 14.477 s | **53.1 agg tok/s** | 0 | 0 |
| MTP on | 29.438 s | **26.1 agg tok/s** | 0 | 0 |

**0.49x** —— MTP 让并发慢一倍。功能是好的（无报错、无截断、输出完整），
纯粹是性能反转。

原因是结构性的，代码里就写着（`runtime/backends/qwen36.py::mtp_verify_and_commit_batch`）：

> Sequential per slot, not a fused batched verify

- MTP off：一个批量 decode step 同时推进 4 个槽（~40 ms 出 4 个 token）
- MTP on：**4 轮串行**，每轮 ~100 ms（~400 ms 出 ~12 个 token）

这正是路线图里的 **M-2（跨槽批量 verify/draft）**，历史实现是**一次跨槽
`verify_batch_spec`**，当年这项 **+43%**。

### (3) 绝对吞吐距历史实现还差得远

| | 配置 | 聚合吞吐 |
|---|---|---|
| 历史（`survey.md:403`） | **128K** / c=4，CG，numpy 优化后 | **174.30 accepted tok/s**，步 74.87 ms |
| 我们（本次） | **短上下文**（提示 15~20 token）/ c=4 | **53.1**（MTP off）/ 26.1（MTP on） |

历史那组是在 **128K 长上下文**下测的，比我们的短上下文难得多，吞吐仍是我们的
**3.3 倍**。c=1 的对照同理：历史 128K 下 33.05 / 40.78 tok/s，我们短上下文
capacity=1 只有 16~32 tok/s（还随热态漂）。

### (4) 所以「达标」应该怎么定义

不能用「比我们自己不开投机快」。至少要：

1. **同配置对历史实现**（同上下文长度、同并发、同 K），而不是自比；
2. **capacity>1 必须是主口径**，因为那才是服务形态。capacity=1 的数字只能
   用来判断单轮结构是否合理，不能当交付指标；
3. 绝对吞吐 + 聚合吞吐都要报，且注明上下文长度与热态（这块 Max-Q 卡同一份
   代码的基线能差 57%，见 §七的测量纪律）。
