...
每轮 2 次全量前向 → 1 次。当前每轮 126 ms(prose)/154 ms(code)，打平线
99.1/117.3 ms，需要补 25~30%；砍掉一次全量前向远超这个量。届时按**当前**
接受率就该净胜，不需要先把接受率拉到历史水平。接受率的提升（M-2 跨槽批处理、
深生成的自然增益）是**叠加**项，不是前提。
