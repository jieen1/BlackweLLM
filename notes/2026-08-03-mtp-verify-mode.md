...
|---|---|---|---|
| prose | 25.41 tok/s（ITL 中位 36.8 ms） | 7.08 tok/s（129.9 ms） | **0.28x** |
| code | 21.36 tok/s（38.5 ms） | 6.83 tok/s（138.9 ms） | **0.32x** |

算术很直接：一轮 154.7 ms 产出 1.54~2.0 个 token → 77~100 ms/token，对比
plain decode 的 36.5 ms。**光叶子内核就 105.6 ms/轮 ≈ 2.9 个 decode step 的
算力**，换 1.54~2.0 个 token。即便接受率提到历史的 ~3.0，也只是打平。
...
同一次 A/B 里，`max_tokens=256`、温度 0：MTP off 吐满 256 个 token，MTP on
只吐 **92 / 97** 个。温度 0 下两条路径的 token 序列必须相同（第一节的验收脚本
正是这么证的），所以这是服务器路径**独有**的提前终止，不是模型行为。尚未定位。
