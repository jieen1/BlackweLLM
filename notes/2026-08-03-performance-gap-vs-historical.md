...
> 🔴 **2026-08-03 晚补充两条更正**（见
> [`2026-08-03-historical-implementation-survey.md`](2026-08-03-historical-implementation-survey.md)）：
>
> 1. **本文下面这个 3.3 → 1.20 的自比自对照仍然成立**，但要知道当年那个 3.3
>    **并不落后于原生**——"原生 4.85" 是 benchmark 双重计数 bug，修正后原生 2.926、
>    我们 3.0。所以"接受长度是差距来源"这个当年的归因本身是错的。
> 2. **本文"原始 decode 慢 1.6×"那节的折算已被实测取代**：
>    128K 实测见 [`2026-08-03-128k-decode-profile.md`](2026-08-03-128k-decode-profile.md)，
>    MTP 一轮的实测分解见 [`2026-08-03-mtp-round-profile.md`](2026-08-03-mtp-round-profile.md)
>    ——真正的瓶颈是**主机侧拷贝与未捕获的 `verify_forward`**，不是 kernel 慢。
>    本文的折算保留存证，但**不要再拿它当依据**。
